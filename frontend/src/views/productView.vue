                    ...
                    <form @submit.prevent="submitProdunctForm" method="post" class="form form-product">
                            ...
                            <template v-if="quiz.isRequired">
                                ...
                                <template v-for="select in   quiz.options" :key="select.id">
                                    ...
                                    <label :for="`quiz${select.selectQuestionId}`"
                                        ...
                                        <input v-model="productInfo.selectQuestionId" true-value="selectQuestionId"
                                            false-value="" type="radio" :name="`quiz${select.selectQuestionId}`"
                                            class="radio">
                                    </label>
                                </template>
                            ...
                            <template v-else>
                                <template v-for="frSelect in   quiz.options" :key="frSelect.id">
                                    <label class="form-control my-2 d-flex justify-content-between">
                                        ...
                                        <input v-model="productInfo.selectQuestionId" :value="`otherAdded${frSelect.id}`"
                        ...
                        <template v-for="quizCustom in  product.customQuestions " :key="quizCustom.id">
                            <label for="" class="">
                                ...
                                <textarea v-model="productInfo.notes" class="form-control form-textarea" name=""
                        ...
                        <label for="" class="py-3 d-flex">
                            ...
                            <select v-model="productInfo.cups" id="" class="ms-3">
                        ...
                        <div class="d-grid py-3">
                            <button type="submit" class="btn btn-submit p-3">
                                新增
                                <span class="totalCount">{{ productInfo.cups }}</span> 產品
                                • 至購物車
                            </button>
                        </div>
                    </form>
...
export default {
    ...
    async created() {
        ...
        // this.product = data;
        // renderData(data);
        // console.log(this.productInfo.cups)
    },
    methods: {
